...
class FileTree(QTreeWidget):
    ...
    def load_file_tree_multi_thread(self, file_datas, root_path):
        self.blockSignals(True)
        self.clear()
        root = self.invisibleRootItem()
        # 先对目录进行分组
        # key: 根目录的名字，value: file_data 列表
        file_datas_by_root_dir = {}
        for file_data in file_datas:
            file_path, match, ext_name, file_size = file_data
            root_dir_name = path_analysis(match)[0]
            if root_dir_name not in file_datas_by_root_dir:
                file_datas_by_root_dir[root_dir_name] = []
            file_datas_by_root_dir[root_dir_name].append(file_data)

        def _build_node(root_dir_name, file_datas, root_path):
            if root_dir_name == 'test.class.py':
                print()
            root = QTreeWidgetItem()
            # 设置路径类型，是目录还是文件
            root.setData(0, Qt.ItemDataRole.UserRole, FileTree.PathType.DIR)
            root.setText(0, root_dir_name)
            root.setFlags(root.flags() | Qt.ItemFlag.ItemIsUserCheckable)
            root.setCheckState(0, Qt.CheckState.Checked)
            # 设置tooltip
            root.setToolTip(0, os.path.normpath(os.path.join(root_path, root_dir_name)))
            if os.path.isfile(os.path.join(root_path, root_dir_name)):
                root.setData(0, Qt.ItemDataRole.UserRole, FileTree.PathType.FILE)
                return root
        ...
        with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
            for root_dir_name, file_datas in file_datas_by_root_dir.items():
                future = executor.submit(_build_node, root_dir_name, file_datas, root_path)
                fs.append(future)
            for future in concurrent.futures.as_completed(fs):
                item = future.result()
                children.append(item)
        children = sorted(children, key=lambda item: item.toolTip(0))
    ...
    def load_file_tree_by_layers(self, layers):
        pass
